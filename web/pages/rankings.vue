...
const url = computed(() => `/api/rankings?source=${sourceSlug.value}`)
const { data, error } = await useFetch<RankingsOverview>(url)
...
<template>
  ...
  <div v-else-if="notYetPublished">
    ...
    <p class="source-empty">
      {{
        sourceSlug === 'fan-poll'
          ? "The Fan Poll hasn't published yet — not enough ballots have been submitted for any weight class. Try another source above, or check back once more ballots come in."
          : `${currentSourceName} hasn't published any rankings yet. Try another source above.`
      }}
    </p>
  </div>
</template>
